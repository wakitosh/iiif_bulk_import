...
class ManifestImportJob extends AbstractJob
{
   ...
   */
    public function perform()
    {
      ...
      // Optional safety: skip extremely large images that often break
          ...
          // Ensure property ids are present for all values (improves reliability).
            ...
            try {
                ...
                $itemId = method_exists($item, 'id') ? $item->id() : null;
                        $logger->info(sprintf(
                            '[IiifBulkImport] Created Item #%s with %d media.',
                            (string) $itemId,
                            count($media)
                        ));
            } catch (\Exception $e) {
              // Fallback: create item with manifest only to avoid network or
              // derivative failures from media.
                ...
                try {
                    ...
                    $itemId2 = method_exists($item2, 'id') ? $item2->id() : null;
                                $logger->info(sprintf(
